...
public class PostfiksIzraz extends Cvor {
	...
	private void provjeri4() {
		...
		if (list1.uniformniZnak.equals("L_UGL_ZAGRADA")) {
			...
			int povecajStog = lista.tipovi.size();
			
			
			try {
				GeneratorKoda.out.write(GeneratorKoda.pozivFunkcije);
				GeneratorKoda.out.write("\tADD R7," + povecajStog*4 + ",R7\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
			GeneratorKoda.call = false;
		}
